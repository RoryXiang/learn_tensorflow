...
def gen_data(size=1000000):
    """
        ...
        输出数据Y：在实践t，Yt的值有50%的概率为1，50%的概率为0，除此之外，如果
        `Xt-3 == 1`，Yt为1的概率增加50%， 如果`Xt-8 == 1`，则Yt为1的概率减少25%，
...
rnn_outputs, final_state = tf.contrib.rnn.static_rnn(cell,
                                                     rnn_inputs,
        ...
        for idx, epoch in enumerate(gen_epochs(num_epochs)):
            ...
            for step, (X, Y) in enumerate(epoch):
                tr_losses, training_loss_, training_state, _ = sess.run(
                    [loss, total_loss, final_state, train_step],
                ...
                if step % 100 == 0 and step > 0:
                    if verbose:
                        print("Average loss at step",
                              step, "for last 100 steps:", training_loss/100)
